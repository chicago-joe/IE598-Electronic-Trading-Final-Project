...
RegressionResult=pd.read_csv('data\\yahoo finance\\lmresult.csv')
#import regression result

#choose which year we will test
testyear=2008
i=testyear-2008

year=str(RegressionResult['year'][i])
...
class PairStrategy(bt.Strategy):
    params = (
        ('m', float(RegressionResult['slope'][i])),#slope
        ('b', float(RegressionResult['intercept'][i])),#intercept
        ('std', float(RegressionResult['sd_res'][i])),
        ('avg', 0.5),
        ('muti', 1),# muti*std+avg
        ('size', 1000)
    )
...
if __name__ == '__main__':
    ...
    print('%s_%.5f%%' % (testyear,(finalvalue-startvalue)/startvalue * 100))
